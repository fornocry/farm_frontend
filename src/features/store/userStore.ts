...
export type UserStore = {
    ...
    apiPlantField: ({fieldID, plant}: { fieldID: number, plant: string }) => Promise<void>
}
    ...
    return ({
        ...
        apiCheckTask: async ({taskId, claim}: { taskId: string, claim: boolean }) => {
            try {
                ...
                set(state => ({
                    ...state,
                    tasks: state.tasks ? state.tasks.map(t =>
                        t.task.ID === taskId ? {...t, ...taskModel} : t
                    ) : [] // Provide a default value if state.tasks is null or undefined
                }));
            } catch (error) {
                console.error('Task check failed:', error);
            }
        },
        apiPlantField: async ({fieldID, plant}: { fieldID: number, plant: string }) => {
            try {
                const field = await APIPlantField(fieldID, plant);

                set(state => {
                    // Ensure userFields is an array, defaulting to an empty array if it's null
                    const userFields = state.userFields || [];

                    const existingFieldIndex = userFields.findIndex(t => t.FieldID === fieldID);
                    if (existingFieldIndex !== -1) {
                        return {
                            ...state,
                            userFields: userFields.map(t =>
                                t.FieldID === fieldID ? {...t, ...field} : t
                            )
                        };
                    } else {
                        return {
